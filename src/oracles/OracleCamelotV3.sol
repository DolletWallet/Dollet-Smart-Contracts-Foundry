...
import { ICamelotV3Pool } from "../interfaces/ICamelotV3.sol";
...
/**
 * @title Dollet OracleCamelotV3 contract
 * @author Dollet Team
 * @notice An oracle for a token that uses Camelot V3 pool data to price it. One of the tokens in the pool must be WETH.
 */
contract OracleCamelotV3 is Initializable, IOracle {
     ...
     * @param _pool Camelot V3 pool address.
     ...
     * @param _newTwapPeriod New TWAP period (in seconds) that will be used to price the token.
     */
    function _setTwapPeriod(uint32 _newTwapPeriod) internal {
    ...
    function _latestAnswer() private view returns (int256) {
        ...
        (int56[] memory _tickCumulatives,,,) = _pool.getTimepoints(_secondsAgos);
