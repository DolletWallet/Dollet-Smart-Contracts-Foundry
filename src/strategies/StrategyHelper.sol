...
import { IStrategyHelper, IStrategyHelperVenue } from "../interfaces/dollet/IStrategyHelper.sol";
import { ISwapRouter } from "@uniswap/v3-periphery/contracts/interfaces/ISwapRouter.sol";
import { BytesLib } from "@uniswap/v3-periphery/contracts/libraries/BytesLib.sol";
import { StrategyHelperErrors } from "../libraries/StrategyHelperErrors.sol";
...
import { IFraxswapV2Router } from "../interfaces/IFraxswapV2.sol";
import { ICamelotV2Router } from "../interfaces/ICamelotV2.sol";
import { ICamelotV3Router } from "../interfaces/ICamelotV3.sol";
import { IBalancerVault } from "../interfaces/IBalancer.sol";
...
/**
 * @title Dollet StrategyHelperVenueCamelotV2
 * @author Dollet Team
 * @notice StrategyHelperVenue that executes swaps on the Camelot V2 venue.
 */
contract StrategyHelperVenueCamelotV2 is IStrategyHelperVenue {
    using AddressUtils for address;

    ICamelotV2Router public immutable router;

    /**
     * @notice Initializes this StrategyHelperVenueCamelotV2 contract.
     * @param _router A Camelot V2 router address.
     */
    constructor(address _router) {
        AddressUtils.onlyContract(_router);

        router = ICamelotV2Router(_router);
    }
    ...
    function swap(
        ...
        external
    {
        ICamelotV2Router _router = router;

        ERC20Lib.safeApprove(_asset, address(_router), _amount);
        _router.swapExactTokensForTokensSupportingFeeOnTransferTokens(
            _amount, _minAmountOut, _parsePath(_path), _recipient, address(0), _deadline
        );
    }

    /**
     * @notice Parses encoded Camelot V2 swap path.
     * @param _path A swap path to decode.
     * @return _tokens A list of token addresses to use during the swap.
     */
    function _parsePath(bytes memory _path) private pure returns (address[] memory _tokens) {
...
/**
 * @title Dollet StrategyHelperVenueCamelotV3
 * @author Dollet Team
 * @notice StrategyHelperVenue that executes swaps on the Camelot V3 venue.
 */
contract StrategyHelperVenueCamelotV3 is IStrategyHelperVenue {
    using SafeERC20Upgradeable for IERC20;
    using AddressUtils for address;

    ICamelotV3Router public immutable router;

    /**
     * @notice Initializes this StrategyHelperVenueCamelotV3 contract.
     * @param _router A Camelot V3 router address.
     */
    constructor(address _router) {
        AddressUtils.onlyContract(_router);

        router = ICamelotV3Router(_router);
    }
    ...
    function swap(
        ...
        external
    {
        ICamelotV3Router _router = router;
        (address _tokenIn, address _tokenOut) = abi.decode(_path, (address, address));

        IERC20(_asset).safeApprove(address(_router), _amount);
...
/**
 * @title Dollet StrategyHelperVenueFraxswapV2
 * @author Dollet Team
 * @notice StrategyHelperVenue that executes swaps on the Fraxswap V2 venue.
 */
contract StrategyHelperVenueFraxswapV2 is IStrategyHelperVenue {
    using AddressUtils for address;

    IFraxswapV2Router public immutable router;

    /**
     * @notice Initializes this StrategyHelperVenueFraxswapV2 contract.
     * @param _router A Fraxswap V2 router address.
     */
    constructor(address _router) {
        AddressUtils.onlyContract(_router);

        router = IFraxswapV2Router(_router);
    }
    ...
    function swap(
        ...
        external
    {
        IFraxswapV2Router _router = router;
    ...
    /**
     * @notice Parses encoded Fraxswap V2 swap path.
     * @param _path A swap path to decode.
     * @return _tokens A list of token addresses to use during the swap.
     */
    function _parsePath(bytes memory _path) private pure returns (address[] memory _tokens) {
...
/**
 * @title Dollet StrategyHelperVenueTraderJoeV1
 * @author Dollet Team
 * @notice StrategyHelperVenue that executes swaps on the TraderJoe V1 venue.
 */
contract StrategyHelperVenueTraderJoeV1 is IStrategyHelperVenue {
    using AddressUtils for address;

    ITraderJoeV1Router public immutable router;

    /**
     * @notice Initializes this StrategyHelperVenueTraderJoeV1 contract.
     * @param _router A TraderJoe V1 router address.
     */
    constructor(address _router) {
        AddressUtils.onlyContract(_router);
    ...
    function swap(
        ...
        external
    {
        ITraderJoeV1Router _router = router;

        ERC20Lib.safeApprove(_asset, address(_router), _amount);
        _router.swapExactTokensForTokens(_amount, _minAmountOut, _parsePath(_path), _recipient, _deadline);
    }

    /**
     * @notice Parses encoded TraderJoe V1 swap path.
     * @param _path A swap path to decode.
     * @return _tokens A list of token addresses to use during the swap.
     */
    function _parsePath(bytes calldata _path) private pure returns (address[] memory _tokens) {
        uint256 _size = _path.length / 20;
